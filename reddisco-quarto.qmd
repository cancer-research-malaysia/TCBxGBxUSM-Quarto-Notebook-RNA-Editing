...
This notebook processes per-sample, per-replicate RNA editing output from RediTools v1 and replicates the manual filtering workflow in a reproducible, scripted form.
...
1.  **Recovery step** — for each replicate, sites in the master annotation set that fell below the upstream 10% frequency filter are recovered from the pre-filter file and re-attached with their actual FREQ and COV values (or 0 if truly absent).
2.  **List 1(a/b)** — intersect replicates within each condition (sites with FREQ \> 0 in both replicates); compute per-site mean FREQ.
3.  **List 2(a)** — intersect siNT and sip150 consistent sites; compute Δ = F(mean)siNT − F(mean)sip150.
4.  **List 2(b)** — sites present in siNT (List 1a) but absent from sip150 (List 1b).
5.  **List 3** — union of List 2(a) with positive Δ and List 2(b): final editing-reduction sites.
...
test_ann <- read_annotated_tsv(
  "input-data/ORL-207/ORL-207-siNT-R1.known_sites_freq10pct_genic.tsv"
)
glimpse(test_ann)
head(test_ann)
nrow(test_ann)
...
test_pre <- read_prefilter_tsv(
  "input-data/ORL-207/ORL-207-siNT-R1-knownEditing-labeled.tsv"
)
glimpse(test_pre)
head(test_pre)
...
# Step 4: load and recover ONE replicate, check what came back
siNT_R1 <- load_replicate("ORL-207", "siNT", "R1", master)
nrow(siNT_R1)
...
# # Step 5: make List 1a manually
# siNT_R2   <- load_replicate("ORL-207", "siNT",   "R2", master)
# sip150_R1 <- load_replicate("ORL-207", "sip150", "R1", master)
# sip150_R2 <- load_replicate("ORL-207", "sip150", "R2", master)

# list1a <- make_list1(siNT_R1, siNT_R2, "siNT")
# nrow(list1a)
# summary(list1a$mean_freq)    # does the frequency distribution look right?

# list1b <- make_list1(sip150_R1, sip150_R2, "sip150")
# nrow(list1b)

# # Step 6: List 2a — how many shared sites? delta distribution sensible?
# list2a <- make_list2a(list1a, list1b)
# nrow(list2a)
# summary(list2a$delta_freq)
# hist(list2a$delta_freq, breaks = 40)   # quick base R plot, no ggplot needed

# # Step 7: List 2b — siNT only sites
# list2b <- make_list2b(list1a, list1b)
# nrow(list2b)
...
Recovered sites (where at least one replicate value came from the pre-filter file) are shown with a different shape so they can be assessed separately.
